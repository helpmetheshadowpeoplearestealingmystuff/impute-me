	...
	output$text_3 <- renderText({ 
		...
		if(input$goButton == 0){
			...
			m<-paste0("<small><br><b>Methods</b><br>
								A per-SNP genetic risk score (GRS) is calculated by counting the risk-alleles multiplied by the effect size (OR or beta as reported in <u><a href='http://www.gwascentral.org/'>GWAS central</a></u>). The per-trait GRS is simply the mean of all per-SNP GRS. Importantly the GRS is here scaled to mean-center and unit-variance relative to the population distribution of possible genotypes, effectively making the scores <u><a href='https://en.wikipedia.org/wiki/Standard_score'>Z-scores</a></u>. This is done using the minor-allele frequency (MAF) for each SNP. If a person is homozygote for a very rare risk variant this will result in a very high GRS Z-score. Further details of the calculation can be found in the <u><a href='https://github.com/lassefolkersen/impute-me/blob/6c05cef7a182c895fa88cd741e4d529e4bfb8200/functions.R#L1023-L1172'>source code</a></u>. The advantage of this approach is that it is robust without need for further data input than MAF. The disadvantage is that particularly ethnicity-specific genetics may be skewed. This is however also the case for the GWAS reported risk-SNPs and effect sizes to begin with, and it is recommended to check the original study to see what ethnicity was investigated. Likewise, in some cases the term genetic <i>risk</i> score may be problematic, as it is not clearly a case of higher values being more risk-related, e.g. vitamin-levels. Again it is recommended to consult with the original GWAS publication.</small>")
